...
Cardioception now runs on **Python 3.10 or 3.11 with PsychoPy 2026.2.2**. We tested this the way a user would: by installing into a clean virtual environment and running the tasks, not simply by checking the dependency metadata. The Python bounds are now declared properly, so pip will reject an unsupported version immediately instead of failing halfway through a build.
...
The larger job in this release was the cardioception documentation. The old site was useful as a reference, but it too often assumed that you already knew what the tasks measured and what to do with their output. The new documentation is organised around the questions people actually bring to it:
...
The tutorials are the part I am most excited about. Each one runs against real fitted output, rather than presenting code that is merely claimed to work:
...
For the modelling itself, the tutorials use the [Hierarchical Interoception toolbox](https://github.com/embodied-computation-group/Hierarchical-Interoception), described in Courtin et al. (2026). It provides hierarchical psychometric models for HRD and RRST data in Stan and brms, tested with parameter recovery. It also includes normative priors from large reference datasets and a power-analysis suite for deciding how many participants and trials you need.
